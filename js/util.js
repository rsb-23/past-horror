...
function getDate(mm_dd) {
    return new Date(base_year + "-" + mm_dd);
}
...
function showBirdInfo(bdate) {
    ...
    var bird = Object.keys(birds).find(x =>
        (bdate >= getDate(birds[x][0]) && bdate <= getDate(birds[x][1]))
    );
    bird = bird.trimEnd();
...
function showTreeInfo(bdate) {
    ...
    var tree = Object.keys(tree_range).find(x =>
        (bdate >= getDate(tree_range[x][0]) && bdate <= getDate(tree_range[x][1]))
    );
    tree = tree.trimEnd()
...
function showZodiacInfo(bdate) {
    ...
    var sign = Object.keys(signs).find(x =>
        (bdate >= getDate(signs[x][0] + "-" + 21) && bdate <= getDate(signs[x][1] + "-" + 20))
    );
    if (sign == null) {
        sign = "Capricorn";
    }
